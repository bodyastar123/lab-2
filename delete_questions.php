...
if (isset($_GET['del'])) { 
    $del = intval($_GET['del']); 
    $query = "DELETE FROM questions WHERE (question_id='$del')"; 
...
// Виводимо дані з таблиці 
  ...
  echo '<tbody>';
	...
	echo '<td>' . $row['q_level'] . '</td>';
	echo '<td>' . $row['q_media'] . '</td>';
	echo '<td>' . $row['media_file'] . '</td>';


        //Генеруємо силку для видалення запису 
        echo "<td><a name=\"del\" href=\"delete_questions.php?del=".$row["question_id"]."\">Видалити</a></td>\n";
        echo "</tr>\n";
    }
echo ("</table>\n");
